...
class AiServiceProvider extends ModuleServiceProvider
{
    ...
    private function registerFrameworkTools(): void
    {
        ...
        $registry->register('delegate_to_agent', 'Delegate To Agent', 'Hand the conversation off to a specialised digital employee; returns a delegate instruction for the frontend. agent_id MUST be a real value from list_agents (long numeric agent_id or role key); never invent short ids. On failure the error returns enabled_agents list: retry ONCE with a real agent_id from it', DelegateToAgentTool::class, ['type' => 'object', 'properties' => ['agent_id' => ['type' => 'string', 'description' => '目标员工的真实 agent_id（长数字，来自 list_agents）或角色标识（如 marketing），不得编造'], 'reason' => ['type' => 'string', 'description' => '转派原因'], 'handoff_message' => ['type' => 'string', 'description' => '带给目标员工的开场消息']], 'required' => ['agent_id']], 'secretary');
        $registry->register('enable_agent', 'Enable Agent', 'Enable a digital employee for the current tenant (create from template if not exists). Write operation: the system automatically shows a confirmation card and only executes after the user confirms; introduce the employee role and model cost in text before calling, and never pair it with ask_user_choice in the same turn', EnableAgentTool::class, ['type' => 'object', 'properties' => ['role' => ['type' => 'string', 'description' => '要启用的数字员工角色标识（如 customer_service / sales / scrm_marketing）']], 'required' => ['role']], 'secretary', 'L2');
        $registry->register('fetch_site_metadata', 'Fetch Site Metadata', 'Fetch a website URL and extract branding metadata (logo, favicon, title, description, theme color, site name) for tenant initialization', FetchSiteMetadataTool::class, ['type' => 'object', 'properties' => ['url' => ['type' => 'string', 'description' => '要抓取的网站 URL（如 https://example.com）']], 'required' => ['url']], 'secretary');
        $registry->register('update_tenant_branding', 'Update Tenant Branding', 'Update current tenant branding settings (team name / description / logo / primary color / secondary color / login page message); pass only fields to change. Prefer this over suggesting form fill when the user asks to set brand info', UpdateTenantBrandingTool::class, ['type' => 'object', 'properties' => ['name' => ['type' => 'string', 'description' => '团队名称'], 'description' => ['type' => 'string', 'description' => '团队介绍'], 'logo' => ['type' => 'string', 'description' => 'Logo 图片 URL'], 'primary_color' => ['type' => 'string', 'description' => '主色调（如 #1890ff）'], 'secondary_color' => ['type' => 'string', 'description' => '辅助色（如 #666666）'], 'login_page_message' => ['type' => 'string', 'description' => '前台登录页欢迎语']]], 'secretary', 'L2');
        $registry->register('update_tenant_settings', 'Update Tenant Settings', 'Update current tenant configuration groups directly: mail (SMTP 邮件发送), auth (登录方式), registration (开放注册/欢迎积分), sms (短信发送). Prefer this over suggesting form fill when the user asks to configure these settings', UpdateTenantSettingsTool::class, ['type' => 'object', 'properties' => ['group' => ['type' => 'string', 'enum' => ['mail', 'auth', 'registration', 'sms'], 'description' => '配置组：mail=邮件发送（SMTP）/ auth=登录方式 / registration=注册设置 / sms=短信发送'], 'settings' => ['type' => 'object', 'description' => '字段键值对，只传需要变更的字段。mail 组：driver/host/port/username/password/encryption(tls|ssl|none)/from_address/from_name；auth 组：allow_phone_login/allow_password_login(1|0)/email_domains；registration 组：allow_register(1|0)/welcome_credits；sms 组：driver/sms_endpoint/sms_access_key/sms_secret_key/sms_sign']], 'required' => ['group', 'settings']], 'secretary', 'L2');
        $registry->register('update_tenant_domain', 'Update Tenant Domain', 'Bind a custom domain to the current tenant (e.g. club.example.com). The domain must have ICP filing; after binding, ownership file verification and platform approval are still required. Prefer this over suggesting form fill when the user asks to bind/set a custom domain', UpdateTenantDomainTool::class, ['type' => 'object', 'properties' => ['domain' => ['type' => 'string', 'description' => '要绑定的自定义域名（如 club.example.com，须已 ICP 备案）']], 'required' => ['domain']], 'secretary', 'L2');
        ...
        $registry->register('ask_user_choice', 'Ask User Choice', 'MANDATORY whenever you need the user to answer a question, confirm something or make a selection (yes/no binary, single or multi select) — e.g. confirming ICP filing before binding a custom domain, choosing among plans. Returns structured {question, options, multiple} rendered as clickable buttons. NEVER ask the user a question in plain text: always call this tool instead', AskUserChoiceTool::class, ['type' => 'object', 'properties' => ['question' => ['type' => 'string', 'description' => '向用户提出的问题（如：club.mtedu.com 是否已完成 ICP 备案？）'], 'options' => ['type' => 'array', 'items' => ['type' => 'string'], 'description' => '选项文案列表（至少 2 项），每项为用户可直接点选的完整答复文案，如 ["是，已完成 ICP 备案", "否，尚未备案"]'], 'multiple' => ['type' => 'boolean', 'description' => '是否允许多选，默认 false（单选）']], 'required' => ['question', 'options']], 'secretary');
