...
class AiServiceProvider extends ModuleServiceProvider
{
    ...
    private function registerFrameworkTools(): void
    {
        ...
        $registry->register('ask_user_choice', 'Ask User Choice', 'Present clickable choice buttons to the user when confirmation or selection is needed (yes/no, single or multi select); returns structured {question, options, multiple} for the frontend to render a choice card instead of asking the user to type', AskUserChoiceTool::class, ['type' => 'object', 'properties' => ['question' => ['type' => 'string', 'description' => '向用户提出的问题（如：club.mtedu.com 是否已完成 ICP 备案？）'], 'options' => ['type' => 'array', 'items' => ['type' => 'string'], 'description' => '选项文案列表（至少 2 项），每项为用户可直接点选的完整答复文案，如 ["是，已完成 ICP 备案", "否，尚未备案"]'], 'multiple' => ['type' => 'boolean', 'description' => '是否允许多选，默认 false（单选）']], 'required' => ['question', 'options']], 'secretary');
